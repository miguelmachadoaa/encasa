<?php
class OlvideController extends Zend_Controller_Action {
    ...
    public function init() {
     ...
     public function indexAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            $usuario=$ObjUsers->fetchRow("email='".$email."'");
            ...
            if ( isset($usuario['id'])) {
                ...
                $update = array('salt' => $codigo );

                $ObjUsers->updateUser($usuario['id'], $update);


                $nombre=$usuario['name'].' '.$usuario['lastname'];

                $message="Se ha solicitado un codigo para reiniciar su cuenta de EnCasaplus.com, Si usted no solicito este codigo ignore este mensaje.El codigo para reinciar su clave es ".$codigo."";

                $asunto="Reinicio de Contraseña | EnCasaPlus";

                $this->generacorreo($email, $nombre, $message, $asunto);

                $this->_redirect('/olvide/codigo');


                
            }else{

                $this->view->error="No se encontro el usuario por favor intente nuevamente";
            }
    ...
    public function codigoAction() {
         ...
         if ($this->getRequest()->isPost()) {
            ...
            $usuario=$ObjUsers->fetchRow("salt=".$codigo."");
            ...
            if ( isset($usuario['id'])) {
               ...
               // $this->_redirect('/olvide/reiniciar');
    ...
    public function reiniciarAction() {
         ...
         if ($this->getRequest()->isPost()) {
            ...
            $reclave=$formData['reclave'];

            if ($clave==$reclave) {
                

            $usuario=$ObjUsers->fetchRow("id='".$formData['id_user']."'");
                ...
                if ( isset($usuario['id'])) {
                    ...
                    $ObjUsers->updateUser($formData['id_user'], $update);
                ...
                $this->view->error="Las claves no coinciden, por favor intente nuevamente";
            }
